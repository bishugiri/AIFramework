...
with tab2:
    ...
    for i, q in enumerate(data_readiness_questions):
        ...
        if score != st.session_state[f"data_readiness_{i}"]:
            st.session_state[f"data_readiness_{i}"] = score
        ...
        if notes != st.session_state[f"data_readiness_notes_{i}"]:
            st.session_state[f"data_readiness_notes_{i}"] = notes
...
with tab3:
    ...
    for i, q in enumerate(infrastructure_questions):
        ...
        # Update session state
        if score != st.session_state[f"infrastructure_{i}"]:
            st.session_state[f"infrastructure_{i}"] = score
        ...
        # Update notes in session state
        if notes != st.session_state[f"infrastructure_notes_{i}"]:
            st.session_state[f"infrastructure_notes_{i}"] = notes
    ...
    for i, survey in enumerate(st.session_state.people_surveys):
        with st.expander(f"📝 {survey['name']} - {survey['role']} ({survey['type']})", expanded=False):
            ...
            for j, q in enumerate(people_questions):
                ...
                # Update session state
                if score != st.session_state[f"people_{i}_{j}"]:
                    st.session_state[f"people_{i}_{j}"] = score
    ...
    for i, survey in enumerate(st.session_state.leadership_surveys):
        with st.expander(f"📝 {survey['name']} - {survey['role']}", expanded=False):
            ...
            for j, q in enumerate(leadership_questions):
                ...
                # Update session state
                if score != st.session_state[f"leadership_{i}_{j}"]:
                    st.session_state[f"leadership_{i}_{j}"] = score
